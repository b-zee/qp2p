...
impl Connection {
    ...
    pub async fn send(&self, user_msg_bytes: WireMsg) -> Result<(), SendError> {
    ...
    pub async fn send_with(&self, user_msg_bytes: WireMsg, priority: i32) -> Result<(), SendError> {
...
impl SendStream {
    ...
    ///
    /// Messages sent over the stream will arrive at the peer in the order they were sent.
    pub async fn send_user_msg(&mut self, user_msg_bytes: WireMsg) -> Result<(), SendError> {
        user_msg_bytes.write_to_stream(&mut self.inner).await
    }
